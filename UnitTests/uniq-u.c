...
/* 2019/04/03  Eliminate the concern that gave rise to warning C4267 in function
               different by changing its return type from int to intmax_t.

			   File open errors report the ERRNO value reported by the CRT
			   library.

			   The output of the main routine opens with displays of the program
			   name and version, followed by a message indicating the platform
			   for which it was built, then the startup date and time (local and
			   UTC). */
...
{
#define ERRMSG_FOPEN					"%s: error opening %s\nError code reported by fopen_s = %#08x (%d decimal)\n"
...
PlatformStartup( )
{
	...
	char * lpFormattedStartTimeUTC = FormatNowUTC( STRFTIME_DEFAULT_UTC_FORMAT ,
		                                            NULL );
