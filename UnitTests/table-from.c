...
static const char* hexbuf( unsigned char buf [ ] , unsigned int buflen )
{
	...
	switch ( buflen )
	{
		case 1: sprintf_s( msg , 50 , "0x%02X" , buf [ 0 ] ); break;
		case 2: sprintf_s( msg , 50 , "0x%02X%02X" , buf [ 0 ] , buf [ 1 ] ); break;
		case 3: sprintf_s( msg , 50 , "0x%02X%02X%02X" , buf [ 0 ] , buf [ 1 ] , buf [ 2 ] ); break;
		case 4: sprintf_s( msg , 50 , "0x%02X%02X%02X%02X" , buf [ 0 ] , buf [ 1 ] , buf [ 2 ] , buf [ 3 ] ); break;
		default: abort( );
	}	// switch ( buflen )

	return msg;
}	// static const char* hexbuf
	...
	else
	{
		if ( inbytesleft != 0 )
		{
			fprintf( stderr ,
				     ...
				     ( long ) ( 3 * sizeof( unsigned int ) - outbytesleft ) );
			exit( 1 );
		}	// if ( inbytesleft != 0 )
			  ...
			  i0++ )
		{
			...
			else
			{
					  ...
					  i1++ )
				{
					...
					{
						const char* unicode = ucs4_decode( out ,
							                               result );
						if ( unicode != NULL )
							printf( "0x%02X%02X\t%s\n" ,
								    i0 ,
